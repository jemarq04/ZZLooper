def combine():
    ...
    for i,key in enumerate(["addr", "vars"]):
        ...
        with open("out/eeee_%s.txt" % key, "r") as infile:
            f_eeee = infile.readlines()

        with open("out/eemm_%s.txt" % key, "r") as infile:
            f_eemm = infile.readlines()

        with open("out/mmmm_%s.txt" % key, "r") as infile:
        ...
        with open("out/%s.txt" % key, "w") as outfile:
            ...
            if i==0:
                outfile.write("}\n")
        print("Combined channels into out/%s.txt" % key)
